...
def trainNetwork(s, readout, h_fc1, s_target, readout_target, h_fc1_target, sess,screen):
    ...
    # define the cost function for target network
    readout_action_target = tf.reduce_sum(tf.mul(readout_target, a), reduction_indices = 1)
    cost_target = tf.reduce_mean(tf.square(y - readout_action_target))
    train_step_target = tf.train.AdamOptimizer(1e-6).minimize(cost_target)
    ...
    while 1:
        ...
        if t > 1000:
            ...
            if t % 5000 == 0:
                train_step_target.run(feed_dict = {
                    y : y_batch,
                    a : a_batch,
                    s_target : s_j_batch})
...
def playGame():
    ...
    trainNetwork(s, readout, h_fc1, s_target, readout_target, h_fc1_target, sess, screen)
