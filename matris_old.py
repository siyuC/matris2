...
class Matris(object):
    ...
    def update(self, timepassed, action):
        ...
        downwards_speed = self.downwards_speed*0.10 if action == 3 else self.downwards_speed
        ...
        try:
            with_tetromino = self.blend(self.rotated(), allow_failure=False)
...
class Game(object):
    def __init__(self):
        self.params = PGNet().params
        self.matris = Matris()  # initialization of environment
        self.PGNet = PGNet()

        """ initialization """
        self.reward = 0
        self.episode_number = 0
        self.rmsprop_cache = { k : np.zeros_like(v) for k,v in self.params.iteritems() } # rmsprop memory


    def train(self, screen, max_iter, learning_rate = 1e-6, decay = 0.995, epsilon = 1, epsilon_inc = 1.005):
        ...
        state = np.zeros((300,600,3))
        state = prepro(state)
        """ training process """
        while self.episode_number < max_iter:
            action = self.PGNet.loss(state, action=None, eps=epsilon)
            dt = clock.tick(45)
            state = self.matris.update((dt / 1000.) if not self.matris.paused else 0, action)# 300*600*3
            print state
            state = prepro(state)# preprocessed state to 1,1,100,100
            state_batch.append(state)
            # score_batch.append(self.matris.score)
            line_batch.append(self.matris.lines)
            action_batch.append(action)
            self.reward += self.matris.lines
 
            if self.matris.gameover: # an episode finished
                self.episode_number += 1
                
                # stack together
                state_batch = np.vstack(state_batch)
                line_batch = np.vstack(line_batch)
                action_batch = np.vstack(action_batch)

                # get grads
                loss, grads = self.PGNet.loss(state_batch, action=action_batch, eps=epsilon, r=line_batch)
                # update weights
                self.rmsprop_cache, self.params = rmsprop_update(self.params, grads, self.rmsprop_cache, learning_rate,  decay)

                # recording
                if self.episode_number % 1 == 0:
                    print 'episode reward now: %2f' %self.reward
                    print action_batch.shape
                    #epsilon *= epsilon_inc
                    #PGNet().save_model(Game().params)
